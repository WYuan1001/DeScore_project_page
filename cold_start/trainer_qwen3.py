...
class Qwen3VLRewardModelBT(Qwen3VLForConditionalGeneration):
    def __init__(self, config, output_dim=4, reward_token="last", special_token_ids=None, return_token_logit=False):
        super().__init__(config)
        # pdb.set_trace()
        self.return_token_logit = return_token_logit
    ...
    def forward(
        ...
        ## modified from the origin class Qwen3VLForConditionalGeneration
        ...
        if self.return_token_logit:
            token_logits = self.lm_head(hidden_states) # [B, L, V]
        ...
        if self.return_token_logit:
            return {'logits': pooled_logits, 'token_logits': token_logits}
        else:
...
class PartialEmbeddingUpdateCallback(TrainerCallback):
    # trainercallback是trainer的一个子类了，可以自由定义在训练刚开始和训练结束的阶段进行一些额外的操作
    """
...
class VideoVLMRewardTrainer(RewardTrainer):
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Union[str, Dataset]] = None) -> DataLoader:
        ...
        data_collator = lambda features: self.data_collator(features)
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if len(lr_mapper) > 0:
                ...
                optim_param_name += [n for n, p in opt_model.named_parameters() if (n in decay_parameters and n not in special_lr_parameters and p.requires_grad)]
                optim_param_name += [n for n, p in opt_model.named_parameters() if (n not in decay_parameters and n not in special_lr_parameters and p.requires_grad)]
                ...
                if visual_parameters: 
                    ...
                    optim_param_name += [n for n, p in opt_model.named_parameters() if (n in decay_parameters and n in visual_parameters and p.requires_grad)]
                    optim_param_name += [n for n, p in opt_model.named_parameters() if (n not in decay_parameters and n in visual_parameters and p.requires_grad)]
                ...
                if merger_parameters: 
                    ...
                    optim_param_name += [n for n, p in opt_model.named_parameters() if (n in decay_parameters and n in merger_parameters and p.requires_grad)]
                    optim_param_name += [n for n, p in opt_model.named_parameters() if (n not in decay_parameters and n in merger_parameters and p.requires_grad)]
            else:
                optimizer_grouped_parameters = [
                    {
                ...
                optim_param_name += [n for n, p in opt_model.named_parameters() if (n in decay_parameters and p.requires_grad)]
                optim_param_name += [n for n, p in opt_model.named_parameters() if (n not in decay_parameters and p.requires_grad)]
            ...
            if self.model.special_token_ids:
                ...
                optim_param_name += ['special token']
        
            optimizer_cls, optimizer_kwargs = self.get_optimizer_cls_and_kwargs(self.args, opt_model)
            print(f'============================= Check Optimizer ==============================')
            print(optim_param_name)
    ...
    def compute_loss(
        ...
        rewards_A, rewards_B = outputs_A['logits'], outputs_B['logits']
        print(f'Check reward A: [{torch.mean(rewards_A)}], Check reward BN: [{torch.mean(rewards_B)}]')
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if isinstance(self.model, PeftModel):
            ...
            super(VideoVLMRewardTrainer, self)._save_checkpoint(model, trial)

    def save_lora_adapter(self, state_dict: dict[str, torch.Tensor], output_dir):
        lora_params_names = [name for name in state_dict.keys() if "lora_" in name]
        print(f'lora params num: [{len(lora_params_names)}]')
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        rm_head_state_dict = {k: v for k, v in state_dict.items() if 'rm_head' in k}
        print(f'======== check rm_head_state_dict {rm_head_state_dict.keys()} ==============')
        if len(rm_head_state_dict) == 0:
            print('[WARNING] rm_head_state_dict is empty! rm_head weights may not be saved correctly.')
        else:
            print(f'======== check rm_head_state_dict {rm_head_state_dict.keys()} ==============')
        torch.save(rm_head_state_dict, os.path.join(output_dir, 'rm_head.pth'))
        
        model_state_dict = {k:v for k, v in state_dict.items() if 'rm_head' not in k}
        lora_path = self.save_lora_adapter(model_state_dict, output_dir)
        if lora_path:
        ...
        if not isinstance(self.model, supported_classes):
            ...
            if isinstance(self.accelerator.unwrap_model(self.model), supported_classes):
                base_model = self.accelerator.unwrap_model(self.model).base_model.model
                base_model.save_pretrained(
                    output_dir, state_dict=model_state_dict, safe_serialization=self.args.save_safetensors
                )
            else:
                logger.info("Trainer.model is not a `PreTrainedModel`, only saving its state dict.")
                if self.args.save_safetensors:
                    safetensors.torch.save_file(
                        model_state_dict, os.path.join(output_dir, SAFE_WEIGHTS_NAME), metadata={"format": "pt"}
                    )
                else:
                    torch.save(model_state_dict, os.path.join(output_dir, WEIGHTS_NAME))
        else:
            base_model = self.model.base_model.model
            base_model.save_pretrained(output_dir, state_dict=model_state_dict, safe_serialization=self.args.save_safetensors)
        ...
        if self.processing_class is not None:
            print(f'Saving self.processing_class ...')
...
def compute_multi_attr_accuracy(eval_pred, metainfo_idxs=None, eval_dims=None, save_path=None) -> Dict[str, float]:
    ...
    metrics.update({
        f"accuracy_{eval_dims}": accuracy,
        f"rewards_chosen_avg_{eval_dims}": rewards_chosen_avg,
        f"rewards_rejected_avg_{eval_dims}": rewards_rejected_avg,
    })

    if save_path is not None and metainfo_idxs is not None:
        df = pd.DataFrame(metainfo_idxs, columns=["metainfo_idx"])
        rewards_A = predictions[:, 0, 0]
        rewards_B = predictions[:, 1, 0]
        df[f"reward_A_{eval_dims}"] = rewards_A
        df[f"reward_B_{eval_dims}"] = rewards_B
