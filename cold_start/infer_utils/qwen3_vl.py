...
def get_rope_index(
    ...
    **kwargs,
    ...
    if video_grid_thw is not None:
        if len(video_grid_thw.size()) == 1:
            video_grid_thw = video_grid_thw.unsqueeze(dim=0)
        video_grid_thw = torch.repeat_interleave(video_grid_thw, video_grid_thw[:, 0], dim=0) # 把num_videos*3的变成(num_videos*T)*3
        video_grid_thw[:, 0] = 1
    ...
    if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
        ...
        for _ in range(image_nums + video_nums):
            ...
            if ed_image < ed_video:
                t, h, w = (
                    image_grid_thw[image_index][0], # image_index表示当前这个image是第几个image
            ...
            text_len = ed - st # ed表示image开始的index，那么st到ed之间的这些tokens就是text token
