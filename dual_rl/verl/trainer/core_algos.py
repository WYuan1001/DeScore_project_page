...
def compute_grpo_outcome_advantage(
    ...
    for idx in id2score:
        ...
        # print(f'===== Check rollout in advantage_cal: [{len(id2score[idx])}] =============') 
...
@register_adv_estimator(AdvantageEstimator.GRPO_TVAR)
def compute_grpo_outcome_tokenvar_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6, **kwargs
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Compute advantage for GRPO, operating only on Outcome reward (with only one scalar reward for each response).
    ...
    Args:
        token_level_rewards: `(torch.Tensor)`
            shape: (bs, 3, response_length) 
        3表示三个通道，每个通道是一种reward的加权方式，分别是all, fmt+sub, fmt+acc）
        response_mask: `(torch.Tensor)`
            shape: (bs, 3, response_length)
        每种reward应该用在哪些token上
        index: `(torch.Tensor)`
            shape: (bs,)
        eps: `(float)`
            epsilon value to avoid division by zero
    ...
    """
    scores = token_level_rewards.sum(dim=-1) / response_mask.sum(dim=-1) # [bs, 3]
    
    def cal_adv(index, scores):
        id2score = defaultdict(list)
        id2mean, id2std = {}, {}
        bsz = scores.shape[0]
        for i in range(bsz):
    ...
    for i in range(scores.shape[-1]):
        scores[:, i] = cal_adv(index, scores[:, i])
    # ddp_break()
    returns = scores.unsqueeze(-1) * response_mask
...
def compute_bt_loss(
    ...
    print(f'Reward in BT-loss: [{rewards.size()}]')
    ground_truth = ground_truth.reshape((-1, 2))[:, 0]
    rewards = rewards.squeeze(dim=-1).view((-1, 2))
    ground_truth = [extract_json_content(str(gt)) for gt in ground_truth]

    rewards_chosen = []
    rewards_rejected = []
    for i, gt in enumerate(ground_truth):
        gt_label = gt.get("gsb_result", "") if isinstance(gt, dict) else gt
        gt_label = 'A' if gt_label == 'Video 1 is better' else gt_label
        gt_label = 'B' if gt_label == 'Video 2 is better' else gt_label

        if gt_label == 'A':
            rewards_chosen.append(rewards[i, 0])
            rewards_rejected.append(rewards[i, 1])
        elif gt_label == 'B':
            rewards_chosen.append(rewards[i, 1])
            rewards_rejected.append(rewards[i, 0])
        else:
            continue  # Tie 或解析失败跳过

    if len(rewards_chosen) == 0:
        # 全部样本都是 Tie，返回 0 loss
        zero = rewards.sum() * 0
        return zero, {"bt_loss": 0.0}

    rewards_chosen = torch.stack(rewards_chosen)    # (n,)
    rewards_rejected = torch.stack(rewards_rejected)  # (n,)
    loss = -nn.functional.logsigmoid(rewards_chosen - rewards_rejected).mean()
    
    return loss, {"bt_loss": loss.detach().item()}
...
def compute_gsb(
    ...
    rewards_chosen = torch.stack(rewards_chosen)
    rewards_rejected = torch.stack(rewards_rejected)
    
    right_gsb = (rewards_chosen > rewards_rejected).sum().item()
    total = len(rewards_chosen)
    
    return right_gsb, total
