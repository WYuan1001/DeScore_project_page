...
from ..workers.fsdp_workers_head import FSDPWorker
...
if torch.cuda.is_available():
    cur_gpu = torch.cuda.get_device_name(0)
    print(f"Current GPU: {cur_gpu}")
    # if 'A800' in cur_gpu:
    #     print("Using A800 GPU, setting NCCL_TOPO_FILE")
    #     os.environ["NCCL_TOPO_FILE"] = "/share/huzhiwen/baidu/topo_a800_hpc_bcc.xml"

def process_dict(batch_dict):
    new_batch_dict = {key:[] for key in batch_dict.keys()}
        ...
        if torch.is_tensor(value):
            if value.dim() == 1:
                ...
                if len(value) == batch_size:
                    value = torch.stack([value, value], dim=-1)
                    value = value.view((-1, ))
            elif value.dim() == 2:
                value = value.view((-1,))
            elif value.dim() >= 3:
                value = value.view(-1, *value.shape[2:])
            new_batch_dict[key] = value
        elif isinstance(value, np.ndarray):
            if isinstance(value[0], dict):
                value_ = []
                for val in value:
                    for kk, vv in val.items(): 
                        value_.append({kk:[vv[0]]})
                        value_.append({kk:[vv[1]]})
                value_ = np.array(value_, dtype=object)
                new_batch_dict[key] = value_
            else:
                if value.ndim == 1:
                    if len(value) == batch_size:
                        print(f'Repeat {key}')
                        value = np.stack([value, value], axis=-1)
...
def process_to_pair_per(object, rollout_batch_size=8, n_split=False):
...
def process_return(batch, rollout_batch_size=None, n_split=False):
    if n_split:
        batch_size = batch.batch['input_ids'].size()[0]*batch.batch['input_ids'].size()[1]
        def restore(v):
            if len(v.size()) >= 4:
                # [n_groups, 8, 2, dim] → [n_groups, 2, 8, dim] → [-1, dim]
                return v.transpose(1, 2).contiguous().view((-1,) + v.size()[3:])
            else:
                ddp_break()
                return v.unsqueeze(2).repeat(1, 1, 2, 1).transpose(1, 2).contiguous().view((-1, v.size()[-1]))
        paired  = {k: restore(v) for k, v in batch.batch.items()}
        ...
        def restore(v):
            if len(v.size()) >= 3:
                # [n_groups*rollout, 2, dim] → [n_groups, 2, 8, dim] → [-1, dim]
                return v.view((-1, rollout_batch_size)+v.size()[1:]).transpose(1, 2).contiguous().view((-1,) + v.size()[2:])
            else:
                # [n_groups*rollout, dim] → [n_groups, rollout, dim] → [n_groups, rollout, 2, dim] → [n_groups, 2, rollout, dim] → [-1, dim]
                return v.view((-1, rollout_batch_size)+v.size()[1:]).unsqueeze(2).repeat(1, 1, 2, 1).transpose(1, 2).contiguous().view((-1,) + v.size()[1:])
        paired  = {k: restore(v) for k, v in batch.batch.items()}
        batch.batch = TensorDict(paired, batch_size=([batch_size*2]))
    
    for key, value in batch.non_tensor_batch.items():
        if n_split:
            batch.non_tensor_batch[key] = value.transpose(0, 2, 1).reshape(-1)
        else:
            batch.non_tensor_batch[key] = value.reshape((-1, rollout_batch_size)).transpose(0, 2, 1).reshape(-1)
    return batch

def remove_special_token_from_batch(data: DataProto, tokenizer) -> dict:
    # 检查是否有 special token
    if 'special_token_ids' not in data.meta_info or data.meta_info['special_token_ids'] is None:
        print("No special token to remove")
        return data
    special_token_id = data.meta_info['special_token_ids']
    special_token_id = [special_token_id] if not isinstance(special_token_id, list) else special_token_id
    
    responses = data.batch['responses'].clone()
    input_ids = data.batch['input_ids'].clone()
    attention_mask = data.batch['attention_mask'].clone()
    response_mask = data.batch['response_mask'].clone()
    position_ids = data.batch['position_ids'].clone() if 'position_ids' in data.batch else None
    prompts = data.batch['prompts']
    
    batch_size, response_length = responses.size()
    prompt_length = prompts.size(1)
    
    print(f"Removing special token (id={special_token_id}) from batch...")
    
    for i in range(batch_size):
        for per_special_token_id in special_token_id:

            special_positions = (responses[i] == per_special_token_id).nonzero(as_tuple=True)[0]
            
            if len(special_positions) > 0:
                special_pos = special_positions[0].item()
                if special_pos < response_length - 1:
                    responses[i, special_pos:-1] = responses[i, special_pos + 1:].clone()
                    response_mask[i, special_pos:-1] = response_mask[i, special_pos + 1:].clone()
                
                pad_token_id = tokenizer.pad_token_id
                responses[i, -1] = pad_token_id
                response_mask[i, -1] = 0
                
                if special_pos < response_length - 1:
                    input_ids[i, prompt_length + special_pos:-1] = input_ids[i, prompt_length + special_pos + 1:].clone()
                input_ids[i, -1] = pad_token_id
                
                if special_pos < response_length - 1:
                    attention_mask[i, prompt_length + special_pos:-1] = attention_mask[i, prompt_length + special_pos + 1:].clone()
                attention_mask[i, -1] = 0
                
                if position_ids is not None:
                    if position_ids.dim() == 2:
                        if special_pos < response_length - 1:
                            position_ids[i, prompt_length + special_pos:-1] = position_ids[i, prompt_length + special_pos + 1:].clone()
                        position_ids[i, -1] = 0
                        
                    elif position_ids.dim() == 3:
                        # Qwen2-VL mrope: (batch_size, 4, seq_len)
                        if special_pos < response_length - 1:
                            position_ids[i, :, prompt_length + special_pos:-1] = position_ids[i, :, prompt_length + special_pos + 1:].clone()
                        position_ids[i, :, -1] = 0
            else:
                print(f"Sample {i}: No special token found")
    
    log_dict = {
        'log_responses': responses,
        'log_input_ids': input_ids,
        'log_attention_mask': attention_mask,
        'log_response_mask': response_mask,
        'log_position_ids': position_ids
    }
    data = data.union(DataProto.from_single_dict(log_dict))
...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0):
    ...
    try:
        response_mask = data.batch['advantage_mask']
    except:
        print(f'Normal condition without advantage_mask, but only response_mask ...')
        response_mask = data.batch['response_mask']
    adv_inputs = {
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.use_bt_as_reward = use_bt_as_reward
    ...
    def _validate(self) -> dict[str, Any]:
        ...
        overall_right_gsb, overall = 0, 0
        for batch_dict in self.val_dataloader:
            process_dict_tag = True if isinstance(batch_dict['multi_modal_data'][0]['videos'], list) else False
            if process_dict_tag:
                    batch_dict = process_dict(batch_dict) 
            ...
            repeat_times = self.config.worker.rollout.val_override_config.get("n", 1) # original version
            ...
            test_gen_batch.meta_info["add_special_token"] = True
            test_gen_batch.meta_info['special_token'] = self.config.data.add_special_token.split(',') #'<Reward>'
            test_output_gen_batch = self.actor_rollout_ref_wg.generate_sequences(test_gen_batch)
            test_output_gen_batch = unpad_dataproto(test_output_gen_batch, pad_size=pad_size * repeat_times)
            # 加入query token, 并得到special token对应的logit
                        
            # repeat to align with repeated responses in rollout
            test_batch = test_batch.repeat(repeat_times=repeat_times, interleave=True)
            test_batch = test_batch.union(test_output_gen_batch)
            test_batch.meta_info['return_special'] = True
            test_batch.meta_info['trainable'] = False
            reward = self.actor_rollout_ref_wg.compute_head_reward(test_batch)
            test_batch = test_batch.union(reward)
            
            if process_dict_tag:
                    test_batch = process_to_pair(test_batch, repeat_times, n_split=True) # batch*1*2
            # evaluate using reward_function
            reward_tensor, reward_metrics = ray.get(self.val_reward_fn.compute_reward.remote(test_batch))
            
            right_gsb, total = compute_gsb(test_batch.batch['head_reward'], test_batch.non_tensor_batch['ground_truth'])
            overall_right_gsb += right_gsb
            overall += total

            if process_dict_tag:
                test_batch = process_return(test_batch, n_split=True)
            ...
            if isinstance(reward_tensor, dict):
                reward_tensor = reward_tensor['reward']
                scores = reward_tensor[:, 0, :].sum(-1).cpu().tolist()
            else:
        ...
        self.val_reward_score = overall_right_gsb/overall
        return {"val/gsb_acc": overall_right_gsb/overall, "val/reward_score": self.val_reward_score, **val_reward_metrics, **val_length_metrics}
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            self.process_dict_tag = True if isinstance(batch_dict['multi_modal_data'][0]['videos'], list) else False

            if self.process_dict_tag:
                batch_dict = process_dict(batch_dict)
            ...
            # generate a batch
            gen_batch.meta_info["add_special_token"] = True
            gen_batch.meta_info['special_token'] = self.config.data.add_special_token.split(',') # '<Reward>'
            gen_batch_output = self.actor_rollout_ref_wg.generate_sequences(gen_batch) # rollout + add special token
            ...
            rollout_batch_size = self.config.data.rollout_batch_size * 2
            if current_batch_size < rollout_batch_size:
                ...
                batch = batch[: rollout_batch_size * self.config.worker.rollout.n]
                if self.process_dict_tag:
                    batch = process_to_pair(batch, self.config.worker.rollout.n, n_split=True)
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                if len(global_token_num.size()) > 1:
                    batch.meta_info["global_token_num"] = global_token_num.view((-1,))
                # compute reward
                ...
                if self.process_dict_tag:
                    batch = process_return(batch, n_split=True)

                batch = remove_special_token_from_batch(batch, self.tokenizer)
                ...
                with timer("adv", timing_raw):
                    if "token_level_scores" not in batch.batch:
                        ...
                        if isinstance(reward_tensor, dict):
                            batch.union(reward_tensor['mask'])
                            batch.batch["token_level_scores"] = reward_tensor['reward']
                        else:
                ...
                # ddp_break()
                # update actor
                batch.meta_info['rollout'] = self.config.worker.rollout.n
                if self.config.trainer.critic_warmup <= self.global_step:
                    with timer("update_actor", timing_raw):
                        actor_output = self.actor_rollout_ref_wg.update_actor(batch) # bt_loss都写在这个里面了
