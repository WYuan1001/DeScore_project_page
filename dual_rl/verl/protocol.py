...
class DataProto:
    ...
    def re_pair(self, indices: torch.Tensor) -> None: 
        # indices_np: [batch, rollout, pair] 
        if self.batch is not None:
            repair_tensors = {
                key: torch.stack([
                    torch.stack([
                        torch.stack([per_batch[i.item(), 0], per_batch[j.item(), 1]], dim=0)
                        for i, j in zip(per_ids[:, 0], per_ids[:, 1])
                    ], dim=0)
                    for per_batch, per_ids in zip(tensor, indices)
                ], dim=0)
                for key, tensor in self.batch.items()
            }
            repair_batch = TensorDict(
                source=repair_tensors,
                batch_size=(self.batch.batch_size[0],),
            )
        else:
            repair_batch = None

        repair_non_tensor_batch = {}
        for key, value in self.non_tensor_batch.items():
            print(f'Key [{key}] size: [{value.shape}]')
        for key, value in self.non_tensor_batch.items():
            # try:
            repair_non_tensor_batch[key] = np.stack([
                np.stack([
                    np.stack([per_value[i.item(), 0], per_value[j.item(), 1]], axis=0)
                    for i, j in zip(per_ids[:, 0], per_ids[:, 1])
                ], axis=0)
                for per_value, per_ids in zip(value, indices)
            ], axis=0)
            # except Exception as e:
            #     print(f'Error key is [{key}], error is [{e}]')

        return DataProto(
            batch=repair_batch,
            non_tensor_batch=repair_non_tensor_batch,
    ...
    def sum(self, dim=0, keepdim=False) -> "DataProto":
        if self.batch is not None:
            sum_tensors = {
                key: tensor.sum(dim=0, keepdim=keepdim)
                for key, tensor in self.batch.items()
            }

            sum_batch = TensorDict(
                source=sum_tensors,
                batch_size=(self.batch.batch_size[0],),
            )
        else:
        ...
        sum_non_tensor_batch = {}
        for key, value in self.non_tensor_batch.items():
            sum_non_tensor_batch[key] = np.sum(value, axis=dim, keepdims=keepdim)

        return DataProto(
            batch=sum_batch,
            non_tensor_batch=sum_non_tensor_batch,
            meta_info=self.meta_info,
        )
    
    # 只针对batch size这个维度，也就是dim=0提供的resize功能哦
    def resize(self, shape, begin_idx=1):
        tensor_data = {k: v.view(shape+v.size()[begin_idx:]) for k, v in self.batch.items()}
        batch = TensorDict(tensor_data, batch_size=shape[0], device=self.batch.device)
        non_tensor_batch = {}
        for k, v in self.non_tensor_batch.items():
            non_tensor_batch.update({k: v.reshape(shape+v.shape[begin_idx:])})
        return DataProto(batch=batch, non_tensor_batch=non_tensor_batch, meta_info=self.meta_info)
    
    def squeeze(self, dim):
        tensor_data = {k: v.squeeze(dim=dim) for k, v in self.batch.items()}
        batch = TensorDict(tensor_data, batch_size=len(tensor_data['input_ids']), device=self.batch.device)
        non_tensor_batch = {}
        for k, v in self.non_tensor_batch.items():
            non_tensor_batch.update({k: v.squeeze(axis=dim)})
        return DataProto(batch=batch, non_tensor_batch=non_tensor_batch, meta_info=self.meta_info)
    
    @staticmethod
    def stack(data: list["DataProto"]) -> "DataProto":
        tensor_data, non_tensor_data = {}, {}
        for k in data[0].batch.keys():
            tensor_data.update({k: torch.stack([item.batch[k] for item in data], dim=0)})
        for k in data[0].non_tensor_batch.keys():
            non_tensor_data.update({k: np.stack([item.non_tensor_batch[k] for item in data], axis=0)})
        batch = TensorDict(tensor_data, batch_size=len(list(tensor_data.values())[0]))

        return DataProto(batch=batch, non_tensor_batch=non_tensor_data, meta_info=data[0].meta_info)
