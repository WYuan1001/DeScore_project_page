...
def rearrange_micro_batches(
    ...
    print(f'==== check effective_seqlen: [{len(effective_seqlen)}]')
    print(f'==== check num_micro_batches: [{num_micro_batches}]')
    if dist.is_initialized():
        num_micro_batches = torch.tensor([num_micro_batches], device="cuda")
        dist.all_reduce(num_micro_batches, op=dist.ReduceOp.MAX, group=dp_group)
        num_micro_batches = num_micro_batches.cpu().item()
    print(f'==== check num_micro_batches after: [{num_micro_batches}]')
    print(f'==== check effective_seqlen after: [{len(effective_seqlen)}]')
...
def rearrange_micro_batches_forpair(
    batch: TensorDict, max_token_len: int, dp_group: Optional[dist.ProcessGroup] = None
) -> Tuple[list[TensorDict], list[list[int]]]:
    """Split the batch into a list of micro_batches, where the max_token_len is smaller than max_token_len
    and the number of valid tokens in each micro batch is well balanced.
    """
    # this is per local micro_bsz
    max_seq_len = batch["attention_mask"].shape[-1] # [batch*rollout, pair, dim]
    assert max_token_len >= max_seq_len, (
        f"max_token_len must be greater than the sequence length. Got {max_token_len=} and {max_seq_len=}"
    )
    effective_seqlen = torch.sum(batch["attention_mask"], dim=(-2, -1))# [batch*rollout, pair, dim] -> [batch*rollout]
    total_seqlen = effective_seqlen.sum().item()
    num_micro_batches = min(len(effective_seqlen), ceildiv(total_seqlen, max_token_len))
    print(f'==== check effective_seqlen: [{len(effective_seqlen)}]')
    print(f'==== check num_micro_batches: [{num_micro_batches}]')
...
def prepare_dynamic_batch(data: DataProto, max_token_len: int, mode='single') -> tuple[list[DataProto], list[list[int]]]:
    """
    ...
    if mode == 'single':
        ...
        batch, batch_idx_list = rearrange_micro_batches_forpair(data.batch, max_token_len=max_token_len)
    else:
        raise Exception(f'Invaid mode: [{mode}]')
